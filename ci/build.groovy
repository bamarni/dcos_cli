...
pipeline {
  ...
  stages {
    stage('Check authorization') {
      ...
      steps {
        user_is_authorized([], '8b793652-f26a-422f-a9ba-0d1e47eb9d89', '#dcos-cli-ci')
      }
    }
          ...
          steps {
            sh '''
              bash -exc " \
                ...
                make binary; \
                mkdir -p build/linux; \
                cp dist/dcos build/linux/"
            '''

            stash includes: "python/lib/dcoscli/build/**", name: "dcos-linux"
          }
        }
          ...
          steps {
            sh '''
              bash -exc " \
                ...
                cp dist/dcos build/darwin/"
            '''

            stash includes: "python/lib/dcoscli/build/**", name: "dcos-darwin"
          }
        }
          ...
          steps {
            bat '''
              bash -exc " \
                ...
                cp dist/dcos.exe build/windows/"
            '''

            stash includes: "python/lib/dcoscli/build/**", name: "dcos-windows"
          }
        }
      }
    }
